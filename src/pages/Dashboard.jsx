...
export default function Dashboard() {
  ...
  // ── Tech Admin / System Admin Dashboard — Welcome ─────────
  if (user?.role === 'technical_admin' || user?.role === 'system_admin') {
    const isSuperAdmin = user.role === 'technical_admin'
    const roleLabel = isSuperAdmin ? 'Super Admin' : 'System Admin'
    const campusLabel = isSuperAdmin ? 'All Campuses' : user.campus

    return (
      <div className="page-enter space-y-5">
        <div>
          <h1 className="text-display text-[var(--color-text-primary)]">Welcome, {user.name?.split(' ')[0] || roleLabel}</h1>
          <p className="text-body text-[var(--color-text-muted)] mt-1">{roleLabel} · {campusLabel}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 stagger">
          ...
          <div className="stat-card border-[var(--color-border)] animate-fade-in">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-green-500/10 rounded-lg flex items-center justify-center"><Shield className="w-5 h-5 text-green-600" /></div>
              <div>
                <p className="text-sm font-bold text-[var(--color-text-primary)]">System Status</p>
        ...
        <div className="card p-5">
          <h3 className="text-sm font-bold text-[var(--color-text-primary)] mb-2">Your Responsibilities</h3>
          <div className="space-y-2">
            {isSuperAdmin ? (
              <>
                <p className="text-xs text-[var(--color-text-secondary)]">• Manage user accounts across all campuses</p>
                <p className="text-xs text-[var(--color-text-secondary)]">• Update school website content (FAQ, programs, about section)</p>
                <p className="text-xs text-[var(--color-text-secondary)]">• Configure system branding (logo, banner, colors)</p>
                <p className="text-xs text-[var(--color-text-secondary)]">• Manage campus configuration and college programs</p>
              </>
            ) : (
              <>
                <p className="text-xs text-[var(--color-text-secondary)]">• Manage user accounts for {campusLabel}</p>
                <p className="text-xs text-[var(--color-text-secondary)]">• Help staff with account issues (password resets, access)</p>
                <p className="text-xs text-[var(--color-text-secondary)]">• Report system issues to the developer</p>
              </>
            )}
          </div>
        </div>
  ...
  if (user?.role === 'admin') {
    ...
    const studentBarData = {
      labels: campusStats.map(c => c.name.replace(' City Campus', '').replace(' Campus', '')),
      datasets: [
        {
          label: 'Basic Ed',
      ...
      <div className="page-enter space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 stagger">
          <div className="stat-card animate-fade-in" style={{ borderLeftColor: '#2563eb' }}>
            ...
            <p className="text-2xl font-bold text-[var(--color-text-primary)]">{stuTotal}</p>
          ...
          <div className="stat-card animate-fade-in" style={{ borderLeftColor: '#16a34a' }}>
            ...
            <p className="text-2xl font-bold text-[var(--color-text-primary)]">{php(payRevenue)}</p>
          ...
          <div className="stat-card animate-fade-in" style={{ borderLeftColor: '#080c42' }}>
            ...
            <p className="text-2xl font-bold text-[var(--color-text-primary)]">{php(payOutstanding)}</p>
            <p className="text-xs text-[var(--color-text-muted)] mt-1">Uncollected</p>
          </div>
        </div>
          ...
          {campusStats.map(c => (
            <div key={c.key} className="card p-4 animate-fade-in" style={{ borderLeft: `4px solid ${primaryColor}` }}>
              ...
              <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs">
                ...
                <div><span className="text-[var(--color-text-muted)]">Enrollments</span><p className="font-bold text-[var(--color-text-primary)] text-base">{c.enrollments}</p></div>
                <div><span className="text-[var(--color-text-muted)]">Basic Ed</span><p className="font-semibold text-[var(--color-text-primary)]">{c.basicStu}</p></div>
                <div><span className="text-[var(--color-text-muted)]">College</span><p className="font-semibold text-[var(--color-text-primary)]">{c.collegeStu}</p></div>
                <div><span className="text-[var(--color-text-muted)]">Income</span><p className="font-semibold text-green-600">{php(c.revenue)}</p></div>
                <div><span className="text-[var(--color-text-muted)]">Balance</span><p className="font-semibold text-amber-600">{php(c.outstanding)}</p></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="card p-5">
            <h3 className="text-sm font-bold text-[var(--color-text-primary)] mb-4">Students by Campus & Department</h3>
            <div style={{ height: 260 }}><Bar data={studentBarData} options={barOptions} /></div>
          </div>
          <div className="card p-5">
            <h3 className="text-sm font-bold text-[var(--color-text-primary)] mb-4">Income Distribution by Campus</h3>
            <div style={{ height: 260 }}><Doughnut data={incomeData} options={doughnutOptions} /></div>
          </div>
        </div>

        <div className="card p-5">
          <h3 className="text-sm font-bold text-[var(--color-text-primary)] mb-4">Enrollments per Campus</h3>
          <div style={{ height: 220 }}><Bar data={enrollBarData} options={{ ...barOptions, plugins: { ...barOptions.plugins, legend: { display: false } } }} /></div>
        </div>
      </div>
    )
  }
